<template>
  <div class="panel">
    <template v-if="store.isReadonly">
      <h4>🔒 网格策略参数（只读）</h4>
      <p class="readonly-tip">观察模式：参数修改与计算入口已隐藏，实时行情、订单簿与报告按只读呈现。</p>
      <div class="grid-info">
        <div class="info-row"><span>下限价格</span><span>¥{{ store.config.lowerPrice }}</span></div>
        <div class="info-row"><span>上限价格</span><span>¥{{ store.config.upperPrice }}</span></div>
        <div class="info-row"><span>网格数量</span><span>{{ store.config.gridCount }}</span></div>
        <div class="info-row"><span>每格资金</span><span>¥{{ store.config.capitalPerGrid.toLocaleString() }}</span></div>
        <div class="info-row"><span>初始资金</span><span>¥{{ store.config.initialCapital.toLocaleString() }}</span></div>
        <div class="info-row"><span>网格间距</span><span>{{ gridStep.toFixed(2) }}</span></div>
        <div class="info-row"><span>总网格资金</span><span>¥{{ totalGridCapital.toLocaleString() }}</span></div>
      </div>
    </template>
    <template v-else>
      <h4>⚙️ 网格策略配置</h4>
...
// 数值口径与交互模式完全一致，避免切换模式后图例/数值漂移
const gridStep = computed(() => (store.config.upperPrice - store.config.lowerPrice) / store.config.gridCount)
const totalGridCapital = computed(() => store.config.gridCount * store.config.capitalPerGrid)
</script>
<style scoped>
...
.readonly-tip{font-size:11px;line-height:1.5;color:#fbbf24;background:#fbbf2410;border:1px solid #fbbf2433;border-radius:4px;padding:6px 8px;margin-bottom:8px}
</style>
